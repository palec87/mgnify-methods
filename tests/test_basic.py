...
Test Data Files:
- SRP237882_taxonomy_abundances_SSU_v5.0.tsv: Sola study taxonomy data
- ERP124424_taxonomy_abundances_SSU_v5.0.tsv: OSD2018 study taxonomy data
These files are used in test_process_analysis_metadata to provide realistic sample IDs.
"""
...
def test_process_analysis_metadata():
    ...
    with patch('mgnify_methods.utils.io.fetch_analysis_metadata') as mock_fetch:
        ...
        # Call the function
        result = process_analysis_metadata('/fake/cache', ds_dict)
        ...
        # 4. Check that the index is 'relationships.run.data.id'
        assert result.index.name == 'relationships.run.data.id'
        
        # 5. Verify study tags are correct for sample runs
        for sample_id in sola_samples:
            assert result.loc[sample_id, 'study_tag'] == 'Sola', f"Sample {sample_id} should have study_tag 'Sola'"
        
        for sample_id in osd_samples:
            assert result.loc[sample_id, 'study_tag'] == 'OSD2018', f"Sample {sample_id} should have study_tag 'OSD2018'"
        
        # 6. Check that all expected run IDs are present
        expected_run_ids = sola_samples + osd_samples
        assert sorted(result.index.tolist()) == sorted(expected_run_ids)
...
def test_fetch_analysis_metadata_cached():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Call the function
        result = fetch_analysis_metadata(tmp_dir, analysisId)
...
def test_fetch_samples_metadata_cached():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Call the function
        result = fetch_samples_metadata(tmp_dir, analysisId)
...
def test_import_taxonomy_summary():
    ...
    # Call the function
    result = import_taxonomy_summary(data_folder, path)
    
    # Assertions
    assert isinstance(result, pd.DataFrame)
    assert result.index.name == 'taxonomy'
...
def test_process_samples_metadata():
    ...
    with patch('mgnify_methods.utils.io.fetch_samples_metadata') as mock_fetch:
        ...
        # Call the function
        result = process_samples_metadata('/fake/cache', ds_dict)
        
        # Assertions
        assert mock_fetch.call_count == 2
        assert result.shape[0] == 4
        assert 'study_tag' in result.columns
        assert result[result['id'] == 'SAMPLE001']['study_tag'].iloc[0] == 'Sola'
        assert result[result['id'] == 'SAMPLE004']['study_tag'].iloc[0] == 'OSD2018'
...
def test_save_config():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Call the function
        result_path = save_config(config, tmp_dir, 'test_config.json')
...
def test_extract_first_date():
    ...
    # Test valid date
    assert extract_first_date('2024-01-15') == '2024-01-15'
    assert extract_first_date('Sample collected on 2024-01-15 at site A') == '2024-01-15'
    assert extract_first_date('2024-01-15/2024-01-20') == '2024-01-15'
    
    # Test invalid/missing date
    assert extract_first_date('no date here') is None
    assert extract_first_date(None) is None
    assert extract_first_date(pd.NA) is None
    assert extract_first_date('') is None
...
def test_process_collection_date():
    ...
    # Call the function
    result, new_columns = process_collection_date(test_data)
